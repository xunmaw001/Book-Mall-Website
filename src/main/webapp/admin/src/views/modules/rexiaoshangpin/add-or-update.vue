    ...
    >
      <el-row>
        ...
        <el-form-item class="select" v-if="type!='info'"  label="品牌" prop="pinpai">
          <el-select v-model="ruleForm.pinpai" placeholder="请选择品牌">
            <el-option
...
export default {
  ...
  methods: {
	...
	this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../tushushangcheng/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `rexiaoshangpin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.rexiaoshangpinCrossAddOrUpdateFlag = false;
                  this.parent.search();
